# SkyRadar

SkyRadar 是一个面向 GitHub Code Search 的代码泄露发现系统。它按规则周期性检索 GitHub 代码内容，记录命中结果，提供复核界面，并通过邮件、钉钉 webhook 或飞书 webhook 发送通知。

- 产品名：SkyRadar
- 后端：FastAPI/ASGI + Gunicorn/Uvicorn + MongoDB + Redis + Huey
- 前端：React + Vite + TypeScript + Tailwind CSS + shadcn/ui
- 部署：Docker Compose 拆分 API、nginx、worker、Redis、MongoDB
- API 文档：`docs/api/openapi.yaml`，可通过 `/api/v1/docs` 暴露 Swagger UI

## 目录

- [能力边界](#能力边界)
- [技术路线](#技术路线)
- [项目架构](#项目架构)
- [快速开始](#快速开始)
- [配置](#配置)
- [开发](#开发)
- [测试与验证](#测试与验证)
- [部署](#部署)
- [安全注意](#安全注意)
- [文档索引](#文档索引)
- [Reference](#reference)

## 能力边界

SkyRadar 当前聚焦 GitHub Code Search 主线：

- 周期性执行 GitHub code search。
- 支持多 GitHub 账号/PAT 配置和搜索配额展示。
- 展示泄露结果、趋势统计、标签统计、详情代码片段和受影响资产。
- 支持结果状态处理：确认、误报、忽略、备注。
- 支持查询规则、资产提取规则、任务周期、黑名单、通知邮箱、SMTP、钉钉 webhook 和飞书 webhook 配置。
- 支持可选 OpenAI 泄露简析：按查询规则开关异步分析结果，可配置为 AI 判断项目有用后才推送 webhook。
- 支持邮件、钉钉 webhook 和飞书 webhook 通知。

不在当前主线内的能力：

- 不把 TruffleHog/Gitleaks 等重扫描器直接放进 API route。
- 不在 FastAPI 请求线程内执行长时间扫描。
- 不默认提供公网多租户、用户权限和审计系统。
- 不把 GitHub PAT、SMTP 密码或 webhook token 展示给前端。

## 技术路线

SkyRadar 的路线按“稳定主链路、增强治理、再扩展扫描源”的顺序推进。

| 阶段 | 目标 | 状态 |
| --- | --- | --- |
| P0 | FastAPI 后端、React 前端、Docker Compose 拆分部署、OpenAPI 契约和基础 smoke | 已落地 |
| P1 | GitHub Code Search 稳定性：rate limit、账号轮转、分页恢复、失败重试、去重 | 进行中 |
| P2 | Baseline 和误报治理：finding fingerprint、首次/最近发现、重复命中收敛 | 规划中 |
| P3 | Target/revision 模型：把 repo、branch、commit、gist 等扫描对象建模 | 规划中 |
| P4 | Event/webhook 增量扫描：GitHub Events 或 webhook 作为受控增量输入 | 规划中 |
| P5 | 高级扫描：GH Archive、force-push、外部 detector harness | 评估中 |

详细后端路线见 `docs/backend/PLAN.md`，前端路线见 `docs/frontend/PLAN.md`。

## 项目架构

```mermaid
flowchart LR
    user[Browser] --> nginx[nginx]
    nginx --> ui[React static assets]
    nginx --> api[FastAPI API]
    api --> mongo[(MongoDB)]
    api --> redis[(Redis)]
    api --> github[GitHub API]
    api --> notify[Mail / DingTalk / Feishu]
    worker[Huey worker] --> redis
    worker --> mongo
    worker --> github
    worker --> notify
```

运行时拓扑：

- `nginx`：对外 HTTP 入口，服务 `client/dist`，并把 `/api` 反向代理到 API service。
- `skyradar`：FastAPI API service，监听容器内 `8888`。
- `worker`：Huey 后台任务消费者，负责周期任务和扫描任务。
- `redis`：Huey broker/cache。
- `mongo`：持久化配置、扫描结果和处理状态。

代码目录：

```text
client/
  src/components/      通用布局和 shadcn/ui 组件
  src/features/        按业务功能组织的前端组合组件
  src/lib/api/         类型化 API client 和响应映射
  src/pages/           页面入口

server/
  api/                 FastAPI routes，按 domain 内聚
  core/                配置、数据库、响应、安全等基础边界
  integrations/        GitHub、邮件、webhook 等外部系统适配
  workers/             Huey worker、调度和后台任务
  tests/               跨 domain 的后端 smoke/contract 测试

deploy/
  nginx/               nginx 站点配置
  supervisor/          容器内进程编排配置
  pyenv/               Python 运行和测试依赖

docs/
  api/openapi.yaml     OpenAPI 契约源
  backend/             后端路线、设计、测试、门禁和风险
  frontend/            前端路线、设计、测试、门禁和风险
```

## 快速开始

推荐用 Docker Compose 启动完整拓扑。
Compose 会从当前源码和 `Dockerfile` 构建 SkyRadar 应用镜像；`SKYRADAR_IMAGE` 只作为本次构建产物 tag，不从远端 registry 拉取 SkyRadar 预构建镜像。

```bash
docker compose up -d
```

默认访问：

- 前端：`http://127.0.0.1/`
- 健康检查：`http://127.0.0.1/api/v1/health`
- 默认启用 nginx Basic Auth；未设置用户名和密码时，容器会随机生成、写入持久化 auth volume，并打印到 nginx 日志。后续重新 build 或重建容器会复用同一组凭据。

查看登录用户名和密码：

```bash
docker compose logs nginx | sed -n '/SkyRadar Basic Auth credentials:/,+3p'
```

如果启动时显式设置了 `SKYRADAR_BASIC_AUTH_USERNAME` 和 `SKYRADAR_BASIC_AUTH_PASSWORD`，日志不会打印随机凭据，直接使用你设置的固定凭据登录。

停止：

```bash
docker compose down
```

修改对外端口：

```bash
SKYRADAR_HTTP_PORT=8080 \
docker compose up -d
```

启用 Swagger UI：

```bash
SKYRADAR_API_DOCS_ENABLED=1 \
docker compose up -d
```

启用后访问：

- Swagger UI：`http://127.0.0.1/api/v1/docs`
- OpenAPI JSON：`http://127.0.0.1/api/v1/openapi.json`

## 配置

常用环境变量：

| 变量 | 默认值 | 说明 |
| --- | --- | --- |
| `MONGODB_URI` | `mongodb://mongo:27017` | MongoDB 连接地址 |
| `MONGODB_DATABASE` | `skyradar` | MongoDB database |
| `MONGODB_AUTH_SOURCE` | `skyradar` | MongoDB 认证库 |
| `REDIS_HOST` | `redis` | Redis host |
| `REDIS_PORT` | `6379` | Redis port |
| `SKYRADAR_HTTP_PORT` | `80` | Compose nginx 对外端口 |
| `SKYRADAR_IMAGE` | `skyradar:latest` | Compose 本地构建产物 tag；不会作为 SkyRadar 远端镜像拉取 |
| `SKYRADAR_MONGO_IMAGE` | `mongo:8.2.7` | Compose MongoDB 镜像 |
| `SKYRADAR_MONGO_VOLUME` | `skyradar-mongo-data` | Compose MongoDB 持久化 volume 名称 |
| `SKYRADAR_REDIS_VOLUME` | `skyradar-redis-data` | Compose 拆分 Redis 持久化 volume 名称 |
| `SKYRADAR_ALL_IN_ONE_REDIS_VOLUME` | `skyradar-all-in-one-redis-data` | Compose all-in-one 内置 Redis 持久化 volume 名称 |
| `SKYRADAR_NGINX_AUTH_VOLUME` | `skyradar-nginx-auth` | Compose 拆分 nginx Basic Auth 凭据 volume 名称 |
| `SKYRADAR_ALL_IN_ONE_NGINX_AUTH_VOLUME` | `skyradar-all-in-one-nginx-auth` | Compose all-in-one Basic Auth 凭据 volume 名称 |
| `SKYRADAR_API_DOCS_ENABLED` | `false` | 是否启用 `/api/v1/docs` |
| `SKYRADAR_OPENAPI_PATH` | `docs/api/openapi.yaml` | OpenAPI 契约文件路径 |
| `SKYRADAR_BASIC_AUTH_ENABLED` | `true` | 是否启用 nginx Basic Auth |
| `SKYRADAR_BASIC_AUTH_USERNAME` | 空 | nginx Basic Auth 用户名；为空时优先读取持久化 auth volume，缺失时随机生成 |
| `SKYRADAR_BASIC_AUTH_PASSWORD` | 空 | nginx Basic Auth 密码；为空时优先读取持久化 auth volume，缺失时随机生成 |
| `OPENAI_ENABLED` | `false` | OpenAI 泄露简析环境兜底开关，可在设置页覆盖 |
| `OPENAI_API_KEY` | 空 | OpenAI API Key 环境兜底；设置页保存的密钥优先生效 |
| `OPENAI_BASE_URL` | `https://api.openai.com/v1` | OpenAI API Base URL 环境兜底 |
| `OPENAI_MODEL` | `gpt-4o-mini` | OpenAI 分析模型环境兜底 |
| `OPENAI_TIMEOUT_SECONDS` | `30` | OpenAI SDK 调用超时时间环境兜底 |

Basic Auth 默认开启。未设置用户名或密码时会优先复用 auth volume 中的凭据；volume 为空时随机生成缺失项并打印到 nginx 容器日志：

```bash
docker compose logs nginx | sed -n '/SkyRadar Basic Auth credentials:/,+3p'
```

生产环境可以显式设置固定用户名和密码；显式设置时优先生效并写回运行时 auth 文件：

```bash
SKYRADAR_BASIC_AUTH_USERNAME=admin \
SKYRADAR_BASIC_AUTH_PASSWORD='change-me' \
docker compose up -d
```

如需重置自动生成的登录凭据，可显式设置新的 `SKYRADAR_BASIC_AUTH_USERNAME`/`SKYRADAR_BASIC_AUTH_PASSWORD`，或在确认不需要保留旧登录信息后更换 `SKYRADAR_NGINX_AUTH_VOLUME` 名称。

仅在可信本地开发环境可显式关闭：

```bash
SKYRADAR_BASIC_AUTH_ENABLED=false docker compose up -d
```

### OpenAI 泄露简析

OpenAI 分析默认关闭。启用方式：

- 在前端 `设置 -> OpenAI 分析` 中配置 API Key、Base URL、模型、自定义 prompt、代码上下文窗口、超时、重试和全局并发量。
- 在 `设置 -> 查询规则` 中为需要自动分析的规则打开 `AI 分析` 开关。
- 如需只推送有价值项目，在 `设置 -> OpenAI 分析` 中打开 `有用才推送`。项目内 `config/ai_analysis_prompt.txt` 和 `config/ai_interests.txt` 作为默认内容来源，用户可在设置页直接编辑并保存这两段文本。

扫描命中并保存结果后，worker 会异步投递分析任务。AI 分析只写入每条结果的 `ai_analysis` 附加状态，不会自动修改安全状态、忽略状态或备注；OpenAI 调用失败、超时或返回非法 JSON 时只记录失败状态，不影响扫描结果保存和通知发送。
开启 `有用才推送` 后，启用 AI 分析的查询规则不会在扫描阶段立即推送 webhook；只有 AI 返回 `is_useful=true` 时才推送 webhook。未开启 AI 分析的规则仍按原逻辑立即推送 webhook。Docker 镜像会把项目内 `config/` 复制到 `/SkyRadar/config`，用于初始化默认文本。

### 数据持久化

Compose 默认使用 named volumes 保存运行数据：

- `skyradar-mongo-data`：MongoDB 配置、扫描结果和处理状态。
- `skyradar-redis-data`：默认拆分拓扑的 Redis/Huey broker 和缓存数据。
- `skyradar-all-in-one-redis-data`：all-in-one 兼容模式的内置 Redis 数据。
- `skyradar-nginx-auth`：默认拆分拓扑自动生成的 nginx Basic Auth 凭据。
- `skyradar-all-in-one-nginx-auth`：all-in-one 兼容模式自动生成的 nginx Basic Auth 凭据。

`docker compose up -d` 会在镜像重新构建或容器重建时复用这些 volume。不要在保留数据的环境执行：

```bash
docker compose down -v
```

如需迁移或复用已有 Docker volume，可在启动前覆盖 volume 名称：

```bash
SKYRADAR_MONGO_VOLUME=existing-mongo-data \
SKYRADAR_REDIS_VOLUME=existing-redis-data \
SKYRADAR_NGINX_AUTH_VOLUME=existing-nginx-auth \
docker compose up -d
```

GitHub PAT 建议使用 Fine-grained PAT：

- 只检索公开仓库：通常不需要额外 repository permission。
- 需要检索私有或组织仓库：只给目标仓库 `Contents: Read-only`。
- 不要授予写权限，例如 `Contents: Write`、`Administration`、`Actions`、`Secrets`。
- 建议使用短期 token，并按部署环境最小化仓库范围。
- 如果组织启用了 SSO/SAML，需要对 token 做组织授权。

钉钉 webhook 示例：

```json
{
  "provider": "dingtalk",
  "webhook_url": "https://oapi.dingtalk.com/robot/send?access_token=<dingtalk-access-token>",
  "secret": "<dingtalk-signing-secret>",
  "domain": "https://your-skyradar.example",
  "enabled": true,
  "test": false
}
```

飞书 webhook 示例：

```json
{
  "provider": "feishu",
  "webhook_url": "https://open.feishu.cn/open-apis/bot/v2/hook/<feishu-hook-id>",
  "secret": "<feishu-signing-secret>",
  "domain": "https://your-skyradar.example",
  "enabled": true,
  "test": false
}
```

`domain` 会用于告警消息中的结果页链接。

## 开发

前端开发：

```bash
cd client
npm install
npm run dev
```

Vite dev server 已配置 `/api` 代理到 `http://127.0.0.1:8888`。

后端本地开发需要先提供本机可访问的 MongoDB 和 Redis，再运行 API：

```bash
cd server
MONGODB_URI=mongodb://127.0.0.1:27017 \
REDIS_HOST=127.0.0.1 \
uv run --no-project --python 3.13 \
  --with-requirements ../deploy/pyenv/requirements-dev.txt \
  gunicorn -w 2 -k uvicorn.workers.UvicornWorker -b 127.0.0.1:8888 api:app
```

后台 worker：

```bash
cd server
MONGODB_URI=mongodb://127.0.0.1:27017 \
REDIS_HOST=127.0.0.1 \
uv run --no-project --python 3.13 \
  --with-requirements ../deploy/pyenv/requirements-dev.txt \
  huey_consumer.py workers.huey -k process -w 4
```

## 测试与验证

前端检查：

```bash
cd client
npm run check
```

后端单测：

```bash
uv run --no-project --python 3.13 \
  --with-requirements deploy/pyenv/requirements-dev.txt \
  pytest
```

后端契约检查：

```bash
uv run --no-project --python 3.13 \
  --with-requirements deploy/pyenv/requirements-dev.txt \
  python scripts/backend_openapi_check.py

uv run --no-project --python 3.13 \
  --with-requirements deploy/pyenv/requirements-dev.txt \
  python scripts/backend_route_coverage.py
```

Docker Compose smoke：

```bash
uv run --no-project --python 3.13 \
  --with-requirements deploy/pyenv/requirements-dev.txt \
  python scripts/backend_compose_smoke.py --fresh-volumes
```

## 部署

默认拆分拓扑：

```bash
docker compose up -d
```

查看默认拆分拓扑生成的登录凭据：

```bash
docker compose logs nginx | sed -n '/SkyRadar Basic Auth credentials:/,+3p'
```

短期单容器兼容模式：

```bash
docker compose --profile all-in-one up -d skyradar-all-in-one mongo
```

查看单容器兼容模式生成的登录凭据：

```bash
docker compose logs skyradar-all-in-one | sed -n '/SkyRadar Basic Auth credentials:/,+3p'
```

构建镜像：

```bash
docker build -t skyradar .
```

使用外部 MongoDB：

```bash
docker run -d \
  --name skyradar \
  -p 80:80 \
  -v skyradar-run-redis-data:/data \
  -v skyradar-run-nginx-auth:/var/lib/skyradar/nginx \
  -e MONGODB_URI=mongodb://username:password@host:27017/skyradar \
  skyradar
```

查看 `docker run` 模式生成的登录凭据：

```bash
docker logs skyradar | sed -n '/SkyRadar Basic Auth credentials:/,+3p'
```

## 安全注意

- `/api/v1/*` 当前没有应用级登录态。nginx Basic Auth 默认开启；部署到不可信网络前还必须配合 HTTPS、VPN、内网或可信反向代理。
- 未显式设置 Basic Auth 凭据时，随机生成或复用的用户名和密码会打印到 nginx/all-in-one 容器日志；生产环境建议保护 Docker 日志访问权限。
- GitHub PAT、SMTP 密码和 webhook token 都属于敏感配置，避免在日志、截图、Issue 或聊天记录中暴露。
- `POST /api/v1/github-accounts` 不应向页面回显原始 token；前端 adapter 会在边界删除敏感字段，后端也应保持敏感字段不回显。
- MongoDB、Redis 和 Docker 基础镜像升级后，需要在目标部署环境复验 `/api/v1/health`、worker 消费和通知链路。

## 文档索引

后端：

- `docs/backend/PLAN.md`：路线
- `docs/backend/PROGRESS.md`：当前状态
- `docs/backend/DESIGN.md`：架构和 domain 边界
- `docs/backend/IMPLEMENTATION_GUIDE.md`：实现规则
- `docs/backend/TESTING.md`：测试策略
- `docs/backend/CHECKLIST.md`：门禁
- `docs/backend/RISKS.md`：风险
- `docs/backend/DECISIONS.md`：ADR
- `docs/backend/REFERENCES.md`：参考资料

前端：

- `docs/frontend/PLAN.md`：路线
- `docs/frontend/PROGRESS.md`：当前状态
- `docs/frontend/DESIGN.md`：产品气质和 UI 规范
- `docs/frontend/IMPLEMENTATION_GUIDE.md`：实现规则
- `docs/frontend/TESTING.md`：测试策略
- `docs/frontend/CHECKLIST.md`：门禁
- `docs/frontend/RISKS.md`：风险
- `docs/frontend/DECISIONS.md`：ADR
- `docs/frontend/REFERENCES.md`：参考资料

## Reference

- [0xbug/Hawkeye](https://github.com/0xbug/Hawkeye) - GitHub code leak monitoring project that inspired SkyRadar.

## 许可证

AGPLv3。详见 `LICENSE`。
