...
- 架构分层、domain 边界、API 人工兼容语义和 sync/async 设计原则写入 `DESIGN.md`，本文只保留执行规则和命令。
...
- 不改变 `/api/*` 路径、HTTP 方法、query/body 参数和 response body，除非明确版本化并同步前端 adapter。
...
- 写接口必须继续兼容 JSON 和历史 form-urlencoded 请求；参数解析保留在 FastAPI route 适配层或后续 schema adapter 中，不再引入 `reqparse`。
...
- 新代码通过 `server/core/` 边界读取配置、数据库句柄、统一 response helper、日志和安全脱敏 helper；不恢复旧全局配置、数据库或 response 兼容入口。
...
- Redis `PING` 和版本记录。
- API service 健康检查和经 nginx 反向代理的 `/api/health` 真实请求。
...
- `SKYRADAR_API_DOCS_ENABLED`：是否启用 `/api/docs` 和 `/api/openapi.json`，默认关闭。
...
- `SKYRADAR_BASIC_AUTH_USERNAME`：nginx Basic Auth 用户名。
- `SKYRADAR_BASIC_AUTH_PASSWORD`：nginx Basic Auth 密码。
...
- `SKYRADAR_BASIC_AUTH_USERNAME` 和 `SKYRADAR_BASIC_AUTH_PASSWORD` 必须同时设置才启用 Basic Auth；只设置其中一个时容器启动失败。
- 未设置 Basic Auth 变量时保持本地开发兼容，不生成 `auth_basic` 配置。
- entrypoint 使用运行时密码生成 `/etc/nginx/.skyradar_htpasswd`，不写入仓库或镜像构建层。
- nginx server 级别启用 Basic Auth，覆盖前端页面、静态资源和 `/api`。
- compose healthcheck 在启用 Basic Auth 时必须使用同一组环境变量携带认证信息。
...
- `client/src/features/**` 中依赖 `/api/*` adapter 的变更
...
- route coverage：当前由 `scripts/backend_route_coverage.py` 比对 `server/api/*/routes.py` 注册路径和 `docs/api/openapi.yaml`。
- 后端 pytest：当前由 `uv run --no-project --python 3.13 --with-requirements deploy/pyenv/requirements-dev.txt pytest` 执行。
- worker smoke。
- Docker build smoke。
- compose smoke：目标覆盖 fresh volume 启动、service health、真实 `/api/health`、静态首页、nginx 配置和日志路径、MongoDB 版本和 Redis/Huey 消费。
...
- `GET /api/health` 不使用 `status/msg/result` 包裹。
- 响应字段为 `github` 和 `mongodb`。
- `github`、`mongodb` 可能是布尔值、对象或错误字符串。
- HTTP `200` 只代表 API 进程可响应，不代表所有依赖健康。

排障顺序：

1. 检查容器是否存活。
2. 访问 `/api/health`，区分 API 进程问题和依赖问题。
...
- 生产默认关闭 `/api/docs`、`/api/redoc` 和 `/api/openapi.json`。
...
- webhook URL 应脱敏展示。
- `GET /api/leakage/code` 返回泄露代码是产品功能，但不得进入普通日志和测试快照。
...
- 后端运行时不依赖 Flask 或 Flask-RESTful；form-urlencoded 兼容由 FastAPI route adapter 保持。
...
- 若为了兼容历史协议绕开 FastAPI/Pydantic 默认行为，必须同步 `DESIGN.md`、`docs/api/openapi.yaml` 和契约测试。
...
- API 行为变化时，更新 `DESIGN.md` 的兼容语义、`docs/api/openapi.yaml` 和契约测试。
