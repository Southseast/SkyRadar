...
- 历史 API 中可缺失的字段必须标为可选。
- 不在页面中用 `any` 绕过不确定字段；先补类型或 adapter。
- API response adapter 负责把后端历史结构转成页面更容易消费的结构。
...
- 所有 `/api/*` 路径集中维护。
- 所有请求参数序列化集中处理。
- 所有 response shape 兼容逻辑集中处理。
- 页面组件不直接拼接复杂 query。
- 页面组件不直接依赖 axios/fetch 的底层错误结构。
- 大多数接口返回 `status/msg/result`；`GET /api/leakage` 额外返回 `total`。
- HTTP status 与 body `status` 不等价。
- 多个 DELETE 接口用 body `status=404` 表示兼容成功语义。
- 写接口必须兼容 JSON 和 form-urlencoded 语义；前端优先使用当前 API client 的封装。
...
- `GET /api/leakage` 必须始终传 `status` query，内容为 JSON 字符串兼容值；裸请求缺少 `status` 可能由后端返回校验错误。
- `GET /api/leakage` 列表响应不包含 `code` 和 `affect`，详情页需要另行请求。
- `PATCH /api/leakage` 提交兼容 payload：`id`、`project`、`ignore`、`security`、`desc`。
- `GET /api/leakage/info` 使用 `id` query 读取详情元信息，adapter 需要容忍缺失字段和空值。
- `GET /api/leakage/code` 返回 base64 编码 `code`，前端负责解码展示，并支持长行、空内容和解码失败状态。

统计和设置接口兼容：

- `GET /api/trend` 提供仪表盘总览和任务状态。
- `GET /api/statistic` 用于 tag、language 等维度聚合，adapter 必须容忍空数组、缺失 `_id`、未知语言和兼容字段。
- GitHub 设置响应不得把原始 `password` 存入页面状态；adapter 必须删除后端写入或删除响应中可能出现的 `password` 字段。
- Query 设置的 `POST` 按 tag 新增或更新；删除规则会关联同 tag 结果，前端必须给出明确确认和反馈。
- Cron 未配置时可能返回兼容错误 body，前端应展示默认值。
- SMTP `password` 只允许作为输入值提交，不得从响应写回页面状态。
- Webhook `GET` 返回掩码 URL 和 `webhook_hash`；删除 webhook 时优先使用 `webhook_hash`，避免携带完整 URL。
- 前端不得展示完整 `access_token`、`sign`、`signature`、`timestamp`、`secret`、`token`。

健康检查：

- `GET /api/health` 不使用 `status/msg/result` 包裹。
- `github` 可能是布尔值或错误字符串，`mongodb` 可能是对象或错误字符串。
...
- 网络错误显示可理解的错误信息。
- 后端返回 `status !== 200/201` 时，优先展示后端 `msg`。
- 表单错误显示在字段附近。
- 页面级错误提供重试或返回操作。
...
compose 默认使用 MongoDB 8.2.7。后端健康检查以当前 FastAPI/ASGI 实现为准，目标环境发布前应复验 `/api/health` 中各子项状态。
