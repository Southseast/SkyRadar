...
compose 默认使用 MongoDB 8.2.7。SkyRadar 应用镜像由 compose 从当前源码构建，`SKYRADAR_IMAGE` 只作为本地构建产物 tag，不从远端 registry 拉取 SkyRadar 预构建镜像。MongoDB、Redis 和 nginx Basic Auth 凭据使用 named volumes 保留数据；保留数据环境不要执行 `docker compose down -v`。Basic Auth 默认启用，未设置凭据时可从 `docker compose logs nginx` 查看随机生成或持久化复用的用户名和密码，重新 build 后凭据保持不变。后端健康检查以当前 FastAPI/ASGI 实现为准，目标环境发布前应复验 `/api/v1/health` 中各子项状态。
