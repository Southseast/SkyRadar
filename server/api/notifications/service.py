...
def send_mail_notice(content):
    smtp_config = worker_repository.get_mail_setting()
    receivers = worker_repository.list_notice_receivers()
    try:
...
def send_webhook_notice(tag, results):
    if not results:
        return
    for webhook_setting in worker_repository.iter_enabled_webhook_settings():
        ...
        try:
            if provider == "feishu":
                content = feishu_integration.build_feishu_search_notice_payload(tag, results, hostname, secret=secret)
                feishu_integration.post_feishu_text(webhook, secret, content, timeout=10)
                continue

            content = dingtalk_integration.build_dingtalk_search_notice_payload(tag, results, hostname)
            dingtalk_integration.post_dingtalk_markdown(webhook, secret, content, timeout=10)
